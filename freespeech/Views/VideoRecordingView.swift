...
class CameraManager: NSObject, ObservableObject {
    ...
    private let sessionQueue = DispatchQueue(label: "freespeech.camera.session")
    private let speechQueue = DispatchQueue(label: "freespeech.camera.speech")
    ...
    private var collectedWordConfidences: [WordConfidence] = []

    var onRecordingComplete: ((URL, String, [WordConfidence]) -> Void)?
    var onReadyToRecord: (() -> Void)?
    var onCannotRecord: (() -> Void)?

    var capturePreset: AVCaptureSession.Preset = .high

    var speechLocale: Locale = Locale.current {
        didSet {
    ...
    private var pendingWordConfidencesForCompletion: [WordConfidence] = []

    func stopRecording() {
        executeOnMain {
            self.pendingTranscriptForCompletion = self.finalizedTranscriptText()
            self.pendingWordConfidencesForCompletion = self.collectedWordConfidences
    ...
    private func processRecognitionResult(_ result: SFSpeechRecognitionResult) {
        ...
        // Collect per-segment confidence data
        let segments = result.bestTranscription.segments
        var updatedConfidences: [WordConfidence] = []
        for segment in segments {
            updatedConfidences.append(WordConfidence(
        ...
        collectedWordConfidences = updatedConfidences
    ...
    private func prepareTranscriptCaptureForRecording() {
        executeOnMain {
            ...
            self.pendingWordConfidencesForCompletion = []
            self.collectedWordConfidences = []
    ...
    func fileOutput(_ output: AVCaptureFileOutput, didFinishRecordingTo outputFileURL: URL, from connections: [AVCaptureConnection], error: Error?) {
        DispatchQueue.main.async { [weak self] in
            ...
            if let error = error {
                ...
                let wordConfidences = self?.pendingWordConfidencesForCompletion ?? []
                self?.pendingTranscriptForCompletion = ""
                self?.pendingWordConfidencesForCompletion = []
                self?.onRecordingComplete?(outputFileURL, finalizedTranscript, wordConfidences)
            }
        }
    }
}
...
struct VideoRecordingView: View {
    ...
    init(
        ...
        onRecordingComplete: @escaping (URL, String, [WordConfidence]) -> Void
    ) {
    ...
    private func toggleRecording() {
        ...
        cameraManager.onRecordingComplete = { url, transcript, wordConfidences in
            onRecordingComplete(url, transcript, wordConfidences)
        }
    ...
    private func closeRecorder() {
        if cameraManager.isRecording {
            cameraManager.onRecordingComplete = { url, _, _ in
